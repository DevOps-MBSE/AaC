...
def get_commands() -> list[AacCommand]:
    ...
    """
    my_cmd = AacCommand(
        "gen-plugin", "Generates an AaC plugin from an AaC model of the plugin", genPlugin
    )
    return [my_cmd]
...
def genPlugin(arch_file: str, parsed_model: dict[str, dict]):
    ...
    if confirmation in ["n", "N"]:
        ...
        plugin_add_command_lines, plugin_impl_lines, impl_names = _åprocess_behaviors(
        ...
        _write_file(
            ...
            "__init__.py",
            True,
            ["# WARNIG - Do not edit this file.  Changes may be overwritten by AaC gen-plugin."],
        )
        # 2) write setup.py
        ...
        _write_file(
            ...
            _generate_plugin_py(
                ...
                impl_names,
                plugin_add_command_lines,
                _process_extensions(parsed_model),
            ),
        )
        ...
        _write_file(
            plug_dir, f"{file_name}_impl.py", False, plugin_impl_header + plugin_impl_lines
        )
...
def _get_plugin_name(models: dict[str, dict], arch_file: str):
    ...
    if len(models.keys()) != 1:
        print(
            f"gen-plugin error [{arch_file}]:  Plugin Arch-as-Code yaml must contain a single model."
        )
        raise RuntimeError
...
def _generate_setup_py(plugin_name: str):
    setup_py_lines = []
    setup_py_lines.append("# WARNING - DO NOT EDIT - YOUR CHANGES WILL NOT BE PROTECTED\n")
...
def _generate_plugin_py(
    plugin_name: str,
    impl_names: list[str],
    plugin_add_command_lines: list[str],
    plugin_get_extension_lines: list[str],
):
    plugin_lines = []
    plugin_lines.append("# WARNING - DO NOT EDIT - YOUR CHANGES WILL NOT BE PROTECTED\n")
    plugin_lines.append(
        "# This file is auto-generated by aac gen-plugin and may be overwritten.\n"
    )
    plugin_lines.append("\n")
...
def _process_extensions(parsed_models: dict[str, dict]):
    ...
    for model_type in parsed_models.keys():
        if (
            "data" in parsed_models[model_type] or "ext" in parsed_models[model_type]
