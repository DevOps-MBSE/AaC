...
class TestLanguageContext(TestCase):
    ...
    def test_get_enum_definition_with_type_when_enum_is_in_core_spec(self):
        core_spec = get_aac_spec()
        test_context = LanguageContext(core_spec)
        ...
        self.assertEqual(primitives_definition, test_context.get_enum_definition_with_type("string"))
        self.assertEqual(behavior_type_definition, test_context.get_enum_definition_with_type("pub-sub"))

    def test_get_enum_definition_with_type_when_enum_is_added(self):
        test_context = LanguageContext()

        values = ["a", "b", "c"]

        [self.assertIsNone(test_context.get_enum_definition_with_type(value)) for value in values]

        enum = create_enum_definition("TestEnum", values)
        test_context.add_definition_to_context(enum)

        [self.assertEqual(enum, test_context.get_enum_definition_with_type(value)) for value in values]
