...
class TestPluginManagement(ActiveContextTestCase):
    ...
    def test_list_plugins(self):
        ...
        self.assertEqual(result.messages[0], self.plugin_names_to_string(get_active_context().plugins))

        result = list_plugins(all=False, active=True, inactive=False)
        self.assertEqual(result.status_code, PluginExecutionStatusCode.SUCCESS)
        self.assertEqual(result.messages[0], self.plugin_names_to_string(get_active_context().get_active_plugins()))

        result = list_plugins(all=False, active=False, inactive=True)
        self.assertEqual(result.status_code, PluginExecutionStatusCode.SUCCESS)
        self.assertEqual(result.messages[0], self.plugin_names_to_string(get_active_context().get_inactive_plugins()))
    ...
    def test_activate_plugin(self):
        ...
        self.assertIn(self.VALID_PLUGIN_NAME, self.plugin_names_to_string(get_active_context().get_inactive_plugins()))
        result = activate_plugin(name=self.VALID_PLUGIN_NAME)
        self.assertIn(self.VALID_PLUGIN_NAME, self.plugin_names_to_string(get_active_context().get_active_plugins()))
        self.assertEqual(result.status_code, PluginExecutionStatusCode.SUCCESS)
