...
class TestValidatorFindings(TestCase):
    ...
    def test_add_specific_findings(self):
        ...
        error_finding = ValidatorFinding(DEFAULT_TEST_DEFINITION, FindingSeverity.ERROR, error_message)
        validator_findings.add_error_finding(DEFAULT_TEST_DEFINITION, error_message)
        self.assertEqual(validator_findings.get_error_findings()[0], error_finding)

        warning_message = "test warning finding"
        warning_finding = ValidatorFinding(DEFAULT_TEST_DEFINITION, FindingSeverity.WARNING, warning_message)
        validator_findings.add_warning_finding(DEFAULT_TEST_DEFINITION, warning_message)
        self.assertEqual(validator_findings.get_warning_findings()[0], warning_finding)

        info_message = "test info finding"
        info_finding = ValidatorFinding(DEFAULT_TEST_DEFINITION, FindingSeverity.INFO, info_message)
        validator_findings.add_info_finding(DEFAULT_TEST_DEFINITION, info_message)
