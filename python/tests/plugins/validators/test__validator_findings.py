...
class TestValidatorFindings(TestCase):
    ...
    def test_add_specific_findings(self):
        ...
        error_finding = ValidatorFinding(DEFAULT_TEST_DEFINITION, FindingSeverity.ERROR, error_message, DEFAULT_FINDING_LOCATION)
        validator_findings.add_error_finding(DEFAULT_TEST_DEFINITION, error_message, "validate thing", 0, 0, 0, 0)
        self.assertEqual(validator_findings.get_error_findings()[0], error_finding)

        warning_message = "test warning finding"
        warning_finding = ValidatorFinding(DEFAULT_TEST_DEFINITION, FindingSeverity.WARNING, warning_message, DEFAULT_FINDING_LOCATION)
        validator_findings.add_warning_finding(DEFAULT_TEST_DEFINITION, warning_message, "validate thing", 0, 0, 0, 0)
        ...
        validator_findings.add_info_finding(DEFAULT_TEST_DEFINITION, info_message, "validate thing", 0, 0, 0, 0)
