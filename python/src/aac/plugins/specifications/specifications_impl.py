...
def spec_validate(architecture_file: str) -> PluginExecutionResult:
    ...
    def validate():
        with validation(parser.parse, architecture_file) as result:
            definitions_dict = definition_helpers.convert_parsed_definitions_to_dict_definition(result.parsed_definitions)
...
def _run_spec_validation(parsed_model: dict):
    ...
    for model_name in parsed_model:
        refs = []
        refs.extend(
            search(parsed_model[model_name], ["spec", "requirements", "parent", "ids"])
        )
        refs.extend(
            search(parsed_model[model_name], ["model", "behavior", "requirements", "ids"])
        )
