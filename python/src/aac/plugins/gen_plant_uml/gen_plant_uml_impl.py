...
def puml_component(
    architecture_file: str, output_directory: str = None
    ...
    def generate_component_diagram():
        with validation(parser.parse_file, architecture_file_path) as result:
            ...
            for root_model_name in _find_root_names(model_types):
                model_component_puml = _get_component_content(
                    model_types[root_model_name], [], model_types
                )
            ...
            if output_directory:
                ...
                return f"Wrote PUML component diagram to {output_file_path}."
    ...
    def generate_sequence_diagram():
        with validation(parser.parse_file, architecture_file_path) as result:
            ...
            for use_case_title in _find_root_names(use_case_types):
                ...
                participants = util.search(
                    use_case_types[use_case_title], ["usecase", "participants"]
                )
                for participant in participants:  # each participant is a field type
                    puml_lines.append(
                        "participant {} as {}".format(
                ...
                # process steps
                steps = util.search(
                    use_case_types[use_case_title], ["usecase", "steps"]
                )
                for step in steps:  # each step of a step type
                    puml_lines.append(
                        "{} -> {} : {}".format(
                            step["source"], step["target"], step["action"]
                        )
                    )
    ...
    def generate_object_diagram():
        with validation(parser.parse_file, architecture_file) as result:
            ...
            for model_name in model_types.keys():
                ...
                for component in util.search(
                    model_types[model_name], ["model", "components", "type"]
                ):
...
def _get_component_content(root, existing, model_types):
    ...
    if len(components) > 0:
        ...
        for component in components:
            ...
            component_puml = _get_component_content(
                model_types[component_type], existing, model_types
            )
        ...
        for input in inputs:
            puml_lines.append(
                "{} -> [{}] : {}".format(input["type"], model_name, input["name"])
            )
        outputs = util.search(root, ["model", "behavior", "output"])
        for output in outputs:
            puml_lines.append(
                "[{}] -> {} : {}".format(model_name, output["type"], output["name"])
            )
