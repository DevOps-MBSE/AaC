...
def _collect_all_plugin_definitions(architecture_file_path: str) -> list[Definition]:
    ...
    try:
        ...
        definition_sources = plugin.get_top_level_fields().get(DEFINITION_FIELD_DEFINITION_SOURCES, []) ### TODO: POPO update ###
...
def _prepare_and_generate_plugin_files(
    ...
    plugin_name = template_properties.get(ROOT_KEY_PLUGIN, {}).get(DEFINITION_FIELD_NAME)   ### TODO: POPO update ###
...
def _gather_command_properties(plugin_definition: Definition) -> list[dict]:
    ...
    def modify_command_input_output_entry(in_out_entry: dict):
        ...
        in_out_entry[DEFINITION_FIELD_NAME] = in_out_entry.get(DEFINITION_FIELD_NAME).removeprefix("--")    ### TODO: POPO update ###
    ...
    for cmd in plugin_definition.get_top_level_fields().get(DEFINITION_FIELD_COMMANDS, []):     ### TODO: POPO update ###
        command = {}

        # First line should end with a period. flake8(D400)
        command_help_text = cmd.get(DEFINITION_FIELD_HELP_TEXT) ### TODO: POPO update ###
        if not command_help_text.endswith("."):
            command_help_text = f"{command_help_text}."

        command_input = cmd.get(DEFINITION_FIELD_INPUT) ### TODO: POPO update ###
        if command_input:
            command[DEFINITION_FIELD_INPUT] = [modify_command_input_output_entry(i) for i in command_input]

        command_output = cmd.get(DEFINITION_FIELD_OUTPUT)   ### TODO: POPO update ###
        if command_output:
            command[DEFINITION_FIELD_OUTPUT] = [modify_command_input_output_entry(o) for o in command_output]

        command_name = cmd.get(DEFINITION_FIELD_NAME)   ### TODO: POPO update ###
        command[DEFINITION_FIELD_NAME] = command_name
        command[DEFINITION_FIELD_HELP_TEXT] = command_help_text
        command[DEFINITION_FIELD_DISPLAY] = cmd.get(DEFINITION_FIELD_NAME, command_name)    ### TODO: POPO update ###
        command[DEFINITION_FIELD_GROUP] = cmd.get(DEFINITION_FIELD_GROUP)   ### TODO: POPO update ###
