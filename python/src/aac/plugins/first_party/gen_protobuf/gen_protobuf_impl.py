...
def _get_model_behavior_input_output_definitions(model_definition: Definition) -> dict[str, Definition]:
    ...
    model_interface_message_names = [field.get(DEFINITION_FIELD_TYPE) for field in model_interface_messages] ### TODO: POPO update ###
...
def _get_embedded_data_structures(schema_definition: Definition, active_context: LanguageContext) -> dict[str, Definition]:
    ...
    definition_names_to_traverse = set(
        [field.get(DEFINITION_FIELD_TYPE) for field in schema_definition.get_top_level_fields().get(DEFINITION_FIELD_FIELDS)] ### TODO: POPO update ###
    )
        ...
        if target_definition:
            ...
            if target_definition.is_schema():
                target_definition_field_types = [
                    ...
                    for field in target_definition.get_top_level_fields().get(DEFINITION_FIELD_FIELDS) ### TODO: POPO update ###
                ]
...
def _to_template_properties_dict(
    ...
    for field in fields:
        field[DEFINITION_FIELD_NAME] = _sanitize_string_to_snake_case(field.get(DEFINITION_FIELD_NAME)) ### TODO: POPO update ###
        if field[DEFINITION_FIELD_TYPE] not in active_context.get_primitive_types():
            field[DEFINITION_FIELD_TYPE] = _sanitize_string_to_pascal_case(field.get(DEFINITION_FIELD_TYPE)) ### TODO: POPO update ###
...
def _get_schema_properties(interface_structures: dict[str, Definition], data_definition: Definition) -> dict[str, any]:
    ...
    """
    active_context = get_active_context()
    definition_fields = data_definition.get_top_level_fields()  ### TODO: POPO update ###
    structure_fields = definition_fields.get(DEFINITION_FIELD_FIELDS) ### TODO: POPO update ###

    definition_name = data_definition.name
    definition_description = definition_fields.get(DEFINITION_FIELD_DESCRIPTION) or "" ### TODO: POPO update ###
    ...
    for field in structure_fields:
        proto_field_name = field.get(DEFINITION_FIELD_NAME) ### TODO: POPO update ###
        proto_field_description = field.get(DEFINITION_FIELD_DESCRIPTION) or ""   ### TODO: POPO update ###
        proto_field_type = field.get(DEFINITION_FIELD_TYPE)  ### TODO: POPO update ###
