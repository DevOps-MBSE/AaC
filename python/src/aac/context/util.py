...
def get_python_module_name(package_name: str) -> str:
    ...
    for item in result.split("."):  # expect name to be dot notation
        if not item[0].isalpha():
            raise LanguageError(f"Invalid AaC name: '{item}' within '{package_name}', the first character must be a letter")
        for sub_item in item.split("_"):  # and name elements may contain _
            if not sub_item.isalnum():  # but things between underscore must be alphanumeric
                raise LanguageError(f"Invalid AaC package name: '{package_name}', the name must be alphanumeric")
    return result
...
def get_python_class_name(name: str) -> str:
    ...
    if not name or name == "":
        raise LanguageError(f"Invalid AaC package name: '{name}', the name must be alphanumeric")
    if not name[0].isalpha():
        raise LanguageError(f"Invalid AaC name: '{name}', the first character must be a letter")
    result = name.replace("-", " ").replace("_", " ").replace(" ", "")
    if not result.isalnum():
        raise LanguageError(f"Invalid AaC name: '{name}', the name must be alphanumeric", None)
    return result
