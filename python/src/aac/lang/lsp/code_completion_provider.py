...
class CodeCompletionProvider:
    ...
    registered_completion_callbacks: dict = attrib(default=Factory(dict), validator=validators.instance_of(dict))

    def __attrs_post_init__(self):
        """Post init hook for attrs classes."""
        self.registered_completion_callbacks[SPACE_TRIGGER] = _handle_space_code_completion

    def get_trigger_characters(self) -> list[str]:
        """Return a list of the currently registered trigger characters."""
        return list(self.registered_completion_callbacks.keys())
    ...
    def handle_code_completion(self, ls: LanguageServer, params: CompletionParams):
        ...
        callback_function = self.registered_completion_callbacks.get(trigger_character)
        if callback_function:
            return callback_function(ls, params)
        else:
            logging.debug(f"Failed to find corresponding code completion function for the registered trigger: {trigger_character} in registered callbacks: {self.registered_completion_callbacks}")
