...
def compute_metadata_structure(definition: Definition, language_context: LanguageContext) -> None:
    ...
    _recursive_meta_structure_build(definition, token_traversal_list, root_yaml_structure, root_node, [root_token.value], language_context)

    definition.meta_structure = root_node


def _recursive_meta_structure_build(definition: Definition, tokens: list[Lexeme], yaml_structure: dict[str, Any], parent_node: StructuralNode, traversed_keys: list[str], language_context: LanguageContext) -> None:
    """Recursively builds the meta structure nodes and attaches them to the parent node."""

    child_nodes = []

    def _create_node(node_type: NodeType, node_token: Optional[Lexeme] = None, defining_definition: Optional[Definition] = None) -> StructuralNode:
    ...
    if isinstance(yaml_structure, list):
        for child_structure in yaml_structure:
            ...
            _recursive_meta_structure_build(definition, tokens, child_structure, list_entry_node, traversed_keys, language_context)
        ...
        for child_value in yaml_structure.values():
            ...
            _recursive_meta_structure_build(definition, tokens, child_value, new_node, node_traversal_keys, language_context)
